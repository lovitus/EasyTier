...
## 1. 显式 secure + Stealth 历史性能异常

**状态：历史候选已复现；最终 v2.6.9 候选未复现同等级下降，继续观察。**
...
最终候选 `63ed38f0` 在相同两台 Linux 3.10 主机、QUIC underlay、单 TCP 流条件下重新测得：

| 模式 | 吞吐 | 相对 plain |
| --- | ---: | ---: |
| plain | `813 Mbit/s` | baseline |
| 运行期派生 secure + Stealth | `751 Mbit/s` | `-7.6%` |
| 显式 secure + Stealth | `656 Mbit/s` | `-19.3%` |

三组均为 0 TCP retransmit。因此早期约 10 倍下降不能作为 v2.6.9 当前性能结论；但显式
secure + Stealth 仍比派生模式低约 12.7%，且 relay/foreign-network 拓扑尚未完成同等级
profiling，本条暂不关闭。
...
- 高吞吐或 relay/foreign-network 场景启用显式 `secure_mode + stealth_mode` 时应先按实际
  拓扑压测；目前没有证据要求全面禁用该组合。
