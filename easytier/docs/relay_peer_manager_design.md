...
- 当前阶段先收敛为“默认路由 / 默认出口”能力，不在同一波次内实现 `relay_exclude`。
- `relay_exclude` 保留为后续本地策略能力，等默认路由方案稳定后再单独讨论与落地。
- 本文后续 TODO 仅用于记录已经确认过的取舍和边界，避免再次从零开始讨论。

## 后续 TODO：本地 relay_exclude 策略

### 目标

- 允许本机临时排除某个节点充当 relay。
- 被排除节点仍允许作为最终目的节点访问，不应因为禁止中继而失去与该节点的直接通信能力。
- 该能力仅影响本机选路，不修改 wire，不向其他节点广播。

### 当前共识

- 新增本地配置 `relay_exclude`，支持虚拟 IP 和 `instance_id` 两种标识。
- 每次路由重算时，将 `relay_exclude` 解析为当前 `PeerId` deny set，后续构图和选路只消费该冻结结果，不在发送热路径临时解析。
- 在 LeastHop / LeastCost 构图时，禁止 deny set 中的节点作为中继下一跳；但保留“源节点到该节点自身”的可达性，以及“该节点作为最终目的地”的收发能力。
- 若唯一可行路径必须经过被排除节点，则按 fail-closed 处理，不应偷偷恢复使用该节点中继。
- 配置热更新后应立即触发本机路由重算，仅影响本机行为。

### 待后续确认的细节

- `instance_id` 或虚拟 IP 无法解析、解析结果为空、或出现歧义时的告警与忽略策略。
- deny set 是否仅作用于 OSPF/直连转发选路，还是也要同步作用于 QUIC/KCP Proxy 等附加下一跳选择逻辑。
- 与默认路由 / exit node 的交互边界：例如本机把某节点设为出口时，是否允许同时把它加入 `relay_exclude`。
- 是否需要 RPC / GUI / 运行状态暴露，还是首版只做本地 CLI / TOML 配置。
- 对 foreign network、P2P-only、credential relay 能力等现有策略是否需要单独定义覆盖关系。
