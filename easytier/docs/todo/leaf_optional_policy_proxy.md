...
- the pinned Leaf smoltcp backend is supplied through the vendored
  `third_party/netstack-smoltcp` crate. Its sole local behavior patch drains
  bytes already accepted by `AsyncWrite` before requesting FIN, preventing
  fast remote EOF from discarding the final response body;
