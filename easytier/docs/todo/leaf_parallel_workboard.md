...
### 2026-07-19 Linux Leaf-owned policy TUN clean candidate

| Workstream | Objective | Build-affecting | Evidence target | Status | Shared candidate |
| --- | --- | --- | --- | --- | --- |
| Locked-source audit | Prove the existing Leaf and Linux route primitives are sufficient without a Leaf fork | no | exact SHA/functions/reference semantics | complete | pending |
| Config and support gate | Add one default-off request that selects only supported Linux TUN mode | yes | TOML/CLI/RPC round-trip and unsupported legacy tests | planned | pending |
| Leaf-owned TUN runtime | Start explicit non-auto Leaf TUN without inherited packet FD | yes | config/readiness/cleanup/generation tests | planned | pending |
| Atomic Linux routing | Leaf primary `/1`, EasyTier TUN lower-priority fail-closed fallback | yes | route ordering, interface loss, rollback and old-kernel evidence | planned | pending |
| Exact artifact validation | Same-SHA legacy/fast functionality, performance and resources | no after freeze | `.37/.38`, `lv1g2/lv1g3`, Android legacy boundary | blocked by implementation | pending |

- Dispatch lock remains CLOSED until the complete Phase 1 snapshot passes `.160` and the candidate manifest is updated with exact results.

## 2026-07-19 Leaf-owned policy TUN candidate
...
- `.160` final Rust no-run/focused suite: passed after the OHOS boundary correction.
- `.160` frontend codegen, 22 focused tests, typecheck and production build: passed.
- `Cargo.lock`, workflow pins and generated-file boundaries: inspected; no tracked lock/workflow/generated delta.
- Current state: ready for one immutable candidate commit/push; Linux and Android workflows must cover the complete batch.

### Validation harness recovery

- PacketBatch-specific product code remains reverted.
- Generic Linux dataplane runner and cross-platform A/B evaluator are restored without reintroducing PacketBatch product code; their real evidence must use the corrected candidate SHA.
- Static/synthetic tooling checks passed. Real exact-artifact runs remain pending Linux workflow completion.

## Corrected candidate after `fe9b68bc`

| Workstream | Objective | Build-affecting | Evidence target | Status |
| --- | --- | --- | --- | --- |
| In-process ownership correction | Preserve existing in-process startup after `LeafConfigOptions` became non-`Copy` | Yes | `.160` feature-unified no-run and focused policy suite | Passed on `.160` |
| Preflight feature coverage | Prevent Android/in-process compiler gaps from reaching GitHub | Validation tooling | Standard preflight compiles `easytier-policy/leaf-inprocess` | Passed on `.160` |
