...
## 2026-07-18 - Demand-allocation performance iterations

- Validated comparator: fairness-only `8201a4a8270a173949e8fa0cf994ac7328aa46b2`, Linux run `29622735384`, Android run `29622735405`. Immediate same-namespace baseline was `846.2 / 878.0 / 868.0 Mbit/s` receive (median `868.0`, zero retransmits). At 1000 idle connections Leaf used `1,310,816 KiB VmSize / 32,300 KiB RSS`, p99 connect was `13.86 ms`, and close returned to 11 FDs. With 1000 idle plus simultaneous TCP/20-Mbit UDP it delivered `425.7 Mbit/s` TCP, zero retransmits and zero UDP loss; 30-second idle Leaf CPU was approximately `0.20%` of one core.
- Rejected chunk candidate `0c8894e280528f382d6689131990fef9e22466e6`: exact Linux run `29623951590` and Android run `29623951560` plus artifact verification passed. Demand `VecDeque<Bytes>` queues used 32 KiB chunks, a 64 KiB per-direction connection limit and 64/32 MiB global byte budget. Unloaded receive median was `518.3 Mbit/s`, `40.3%` below comparator. It reduced 1000-idle Leaf to `673,940 KiB VmSize / 26,080 KiB RSS` and returned to 11 FDs, proving the memory direction but failing performance. Literal revert: `e26c9815`.
- Rejected restored-capacity chunk candidate `6c92be286802a5f10ebaa65318e5e830218832f4`: exact Linux run `29624958371` and Android run `29624958377` plus artifact verification passed. It restored both Stream maximums to `0x3fff * 20` while retaining chunk allocation. Unloaded receive median was still only `536.9 Mbit/s`, `38.1%` below comparator with zero retransmits. This isolates per-chunk allocation/free and permit accounting as the hot-path regression rather than the maximum queue capacity. Literal revert: `728a8679`.
- Next implementation: retain smoltcp and Stream capacity at `0x3fff * 20`, but wrap each original Stream `RingBuffer` in lazy storage. The first active read/write direction acquires one capacity block from its per-direction global budget and allocates the original ring once; steady-state enqueue/dequeue remains the old RingBuffer path with no chunk allocation or byte-permit operation. Empty rings are retained while unpressured and reclaimed only when another connection waits for a global block. The budget is block-granular and hard-capped below 64 MiB desktop / 32 MiB Android; newly established idle connections allocate no Stream payload.
- Lifecycle boundary: bounded listener/socket-creation channels remain required. Send waiters use `PollSemaphore`; receive pressure is counted explicitly because the runner cannot await while scanning smoltcp sockets. Close/drop cancels waiter accounting and releases unreachable buffers; received bytes before FIN remain readable. Pressure-only reclamation yields once so a woken connection can claim the block before more idle rings are evicted. The mandatory matrix remains `.160` locked no-run/full netstack tests, exact optimized throughput, 1/100/1000 memory/p99/close, 1000-idle TCP+UDP, budget saturation, and Android persistence/policy/lifecycle/logging/resource checks.
- Dependency evidence: EasyTier locks smoltcp to `0a926767a68bc88d5512afefa7529c5ecdade4ea`; the RingBuffer API was checked in the exact `/tmp/netstack-smoltcp-0a926767` worktree. The Leaf reference remains locked to `4af133266367bc6ef1d369b4b519a0a56da48760` and was inspected at `/private/tmp/leaf-audit-4af133266-v2`.
- `.160` gate after the reclaim-request cancellation correction: `scripts/leaf-remote-preflight.sh` passed one locked no-run build for all three library targets and every focused test. Exact netstack binary `/workspace/target/debug/deps/netstack_smoltcp-e8801535fabfa266` passed all 16 tests serially, including persistent reclaim requests, global-budget wakeup/cancellation, receive-pressure accounting, half-close byte ordering, bounded creation queues, output shutdown and cooperative immediate polling. Optimized throughput and pressure performance remain unproven until exact workflow artifacts are measured; unit-test success is not a performance claim.

### Candidate manifest: lazy-ring stream staging after `728a8679`

- Frozen build-affecting snapshot: parent `728a867922a9e7339aa8d9d3752e0b057d3e0e28`; Rust plus preflight-script diff SHA-256 `3db6ba4f7b5acf34ea71dd77e248280b9984f720bc44f1581648664791b392a3`. Included scope is lazy original-capacity Stream rings, block-granular send/receive budgets, pressure-only empty-ring reclamation, cancellation-safe wakeup accounting, bounded creation queues, and allocation outside the send-side spin mutex.
- `.160` dispatch lock: final post-format snapshot passed `scripts/leaf-remote-preflight.sh`; locked no-run exit `0`; exact EasyTier, policy and netstack focused suites all passed serially. `Cargo.lock`, platform `cfg`, generated files and workflow pins are unchanged.
- Required workflows: push the revert of rejected `6c92be28` and this candidate together once to `codex/profiling-beta`; accept only the automatically started Linux and Android pair for the final candidate SHA and never manually duplicate them.
- Linux evidence: verify artifact SHA/build metadata, then gate first on three unloaded 8-second throughput runs (median must be at least `824.6 Mbit/s`), followed only on pass by 1/100/1000 connection memory/p99/cleanup, 30-second idle CPU, 1000-idle concurrent TCP/UDP, and budget-saturation progress/fairness.
- Android evidence: only after Linux passes, preserve stores and use `adb install -r`; verify stores before first start, captured-UID policy traffic, runner restart cleanup, zero-growth logging, CPU/RSS/FD/tasks, connection pressure and unchanged TCP/UDP/FakeDNS/HEV behavior.
- Workflow-wait work: prepare isolated Linux fixtures and checksums plus Android persistence/probe commands without changing the immutable candidate source.
