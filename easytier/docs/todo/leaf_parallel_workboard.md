...
## 2026-07-20 demand-driven fallback selector candidate

- EasyTier parent is exactly `844b5256b88441e8f5cfed9d95a983af4b9edfea`, tree `ad29539df963ed2deab862ab9ee4e18fafbccb12`; locked Leaf parent is exactly `7f4dc745a2d325e672fff2553c57f9f560d87dd5`.
- Leaf candidate is `013a1497dd29355a00cd776628ff2de72e02e861` on `codex/fallback-selector`; its `.160` library no-run passed and focused selector/config/test-fixture/data-path tests passed 10/10 before the EasyTier pin update.
- Build-affecting scope: replace Leaf `stableFailover` request-failure/outage state machine with one TCP/UDP-shared selector; add fallback-only optional `url`; demand-triggered HTTP(S) checks, singleflight, five-minute freshness, bounded concurrency 10, three-round switch/failback confirmation, and `15s -> 60s -> 5min` inconclusive backoff.
- Explicit invariants: every user request uses only current `selected`; user destination results never alter health; all-fail never changes selection; unused groups create no checks; nested health probes do not recursively trigger groups; ordinary non-stable Leaf failover remains unchanged.
- Reference: Mihomo `/Users/fanli/Documents/mihomo-rev/adapter/outboundgroup/fallback.go::{findAliveProxy,DialContext,ListenPacketContext}`, `/Users/fanli/Documents/mihomo-rev/adapter/provider/healthcheck.go::{check,execute}`, `/Users/fanli/Documents/mihomo-rev/adapter/adapter.go::Proxy::URLTest`, and `/Users/fanli/Documents/mihomo-rev/constant/adapters.go::DefaultTestURL`.
- Intentional difference: EasyTier does not run a startup or periodic checker. A real group touch starts one stale asynchronous session; default URL is Mihomo's `https://www.gstatic.com/generate_204`, with user group-level override.
- `.160` gate: regenerate Leaf protobuf from the exact fork snapshot; commit/pin that Leaf SHA; run `scripts/leaf-remote-preflight.sh` with locked no-run plus exact policy and Leaf fallback tests. Inspect `Cargo.lock`, generated protobuf, platform cfg, workflow pins, and complete diffs before push.
- `.160` evidence: `scripts/leaf-remote-preflight.sh` completed its `--locked` no-run build in 4m23s and all configured focused tests passed. The standalone locked Leaf test binary passed 10/10 selector, stream/datagram no-retry, config and GeoSite fixture tests.
- Frontend evidence: after replacing only platform-stale remote `node_modules` with `pnpm install --frozen-lockfile`, `policy-document.spec.ts` passed 16/16 and the frontend-lib TypeScript/Vite production build passed under Node 22.
