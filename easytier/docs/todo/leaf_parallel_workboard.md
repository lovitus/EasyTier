...
## 2026-07-18 progress-safe adaptive Stream candidate

- Parent snapshot: `fcd35be380c6985329b85aba0ee0a861f3d2f668`; build affecting; not dispatched. Locked smoltcp is `0a926767a68bc88d5512afefa7529c5ecdade4ea`, inspected at `/tmp/netstack-smoltcp-0a926767`. Locked Leaf remains `4af133266367bc6ef1d369b4b519a0a56da48760`, inspected at `/private/tmp/leaf-audit-4af133266-v2`.
- Reference semantics: Mihomo `common/net/sing.go::Relay` uses bounded pooled relay buffers and propagates backpressure; `common/pool/buffer_standard.go::RelayBufferSize` fixes the per-copy relay unit. EasyTier intentionally retains its pinned smoltcp/RingBuffer API instead of importing Mihomo's system-socket relay because the mesh fallback terminates local TCP in userspace. The observable contract followed is bounded memory plus `Pending` backpressure without byte loss, reordering, kernel fallback, or connection-to-connection progress dependency.
- Candidate structure: each smoltcp RX/TX window is `128 KiB` (the optimized 32 KiB experiment was rejected at `-53.8%` throughput). Stream staging is absent while idle. On first activity a direction preferentially receives the original `0x3fff * 20` ring when a shared expansion slot is available; otherwise it receives an unconditional `32 KiB` base ring. Expanded allocations consume the 64 MiB desktop / 32 MiB Android global budget at their full 320 KiB size, so base-to-expanded migration remains within the declared hard bound even while both allocations briefly coexist.
- Progress invariant: a finite global budget may reduce burst capacity but can never prevent a connection from accepting or draining through its independent base ring. A full base ring waits simultaneously for ordinary socket drain and an optional expansion permit; drain cancels the expansion wait and proceeds. Empty expanded rings are reclaimed only while a live waiter exists. This is the explicit correction for rejected `271351f0`.
- Queue/lifecycle boundary: listener and socket-creation channels are bounded by the configured ingress capacity. Close/drop removes exact waiter accounting and unreachable storage; accepted send bytes drain before FIN, and received bytes remain readable before EOF. Normal no-pressure runner scans perform no per-socket reclaim checks; reclamation scans exist only while expansion waiters are live.
- Required `.160` evidence: locked no-run for EasyTier, policy and netstack libraries; adaptive migration/order, zero-slot base progress, expansion wake/cancellation, half-close, bounded queues, output close and cooperative runner tests; complete diff/lock/cfg/workflow-pin review.
- Required exact-artifact gates: unloaded three-run median at least `824.6 Mbit/s`; 1/100/1000 idle memory/p99/close; 30-second idle CPU; same-window 1000-idle TCP/UDP A/B; 128-parallel forward and reverse each complete in the configured eight-second interval; Android persisted upgrade, captured-UID policy, lifecycle, cleanup, logging and resource checks. Any saturation timeout requires literal revert.
- Frozen build-affecting diff SHA-256: `7eac42f7eed82c6d8afb084f83d16af940a4d59bcb3534ef81179d1a0fed39b5`. Final `.160` dispatch lock passed after full-allocation budget accounting and coalesced one-reclaim-per-pressure-pass correction: locked no-run exit `0`; exact EasyTier, policy and netstack binaries passed every focused test serially, including 128-stream-over-one-slot initial progress, zero-slot repeated base progress, migration ordering, multi-waiter reclaim, shutdown cancellation, half-close bytes, bounded queues, output close and cooperative polling. `Cargo.lock`, platform `cfg`, generated files and workflow pins are unchanged.
