...
## 2026-07-18 - Demand-allocated Leaf Stream queue candidate

- Shared base: fairness-only `8201a4a8270a173949e8fa0cf994ac7328aa46b2`; Linux workflow `29622735384` and Android workflow `29622735405` succeeded, and the exact Linux release/Android workflow artifacts passed commit, run, target, HEV-pin and SHA-256 verification. No implementation from the rejected 32 KiB window candidate remains.
- Locked Leaf source: `Cargo.lock` pins `4af133266367bc6ef1d369b4b519a0a56da48760`; exact detached worktree `/private/tmp/leaf-audit-4af133266-v2`, `leaf/src/proxy/tun/inbound.rs::new_smoltcp`, constructs one `StackBuilder`/`TcpListener` for the policy TUN, spawns each accepted `TcpStream` into the dispatcher, and closes the surrounding inbound futures when the TUN lifecycle ends. The byte budget is therefore global to every TCP connection in that Stack/TUN instance; multiple independently configured TUN inbounds intentionally receive independent budgets.
- Mihomo source: HEAD `0a87b94845ef908c15f8495871e4cd8e33116328`. `common/net/sing.go::Relay` performs bounded bidirectional copy, preserves half-close with `CloseWrite`, and closes both endpoints on completion/error. `common/pool/buffer_standard.go::RelayBufferSize` uses 32 KiB relay chunks. Pinned sing-tun v0.4.17 commit `00e7bcea347af9d3b274491f77921827a678b50e`, `stack_gvisor.go::NewGVisorStackWithOptions`, uses separate TCP-stack buffering and moderate receive buffering. Observable compatibility target is bounded relay staging, natural TCP backpressure, exact byte order, half-close preservation, and deterministic cleanup; it is not identical gVisor window sizing.
- Candidate structure: preserve both smoltcp socket buffers at exactly `0x3fff * 20` bytes because same-window evidence proves they are throughput-sensitive. Replace only the two fixed Leaf Stream rings with demand-allocated `VecDeque<Bytes>` chunks. Use 32 KiB chunks, a 64 KiB per-connection/per-direction limit, and two direction-reserved semaphore pools whose combined hard cap is 64 MiB on desktop/gateway and 32 MiB on Android. Direction reservation prevents send-heavy traffic from permanently starving receive staging while preserving the stated total hard bound.
- Budget semantics: each queued chunk owns byte-counted semaphore permits; partial consumption releases exactly the consumed permits, queue/control/stream drop releases the remainder, and a globally exhausted AsyncWrite returns `Pending` through `PollSemaphore`. Per-connection exhaustion uses the existing send waker. Socket close cancels any pending budget acquisition and returns `BrokenPipe`; receive data already accepted before FIN remains readable before EOF. No unbounded byte or connection-creation channel is allowed.
- Intentional boundary: this candidate reduces the duplicate Stream allocation, not smoltcp's fixed address-space reservation. Based on the rejected candidate evidence, claiming approximately 126 MiB total worst-case memory at 1000 high-throughput connections would require a dynamically resizable TCP window that smoltcp 0.13 does not expose and would reintroduce the measured 53.8% throughput loss if approximated by a fixed 32 KiB window. Expected empty-connection virtual memory is roughly halved, while dynamic Stream payload pages remain globally capped; exact RSS/VmSize must be measured rather than inferred.
- Required `.160` tests: unchanged socket-window capacity; no allocation for empty queues; exact partial/full permit reclamation; global-budget Pending/wake/cancel; per-connection Pending/wake; close/drop and half-close cleanup; bounded listener/socket-creation channels; existing output fairness, output-close and Tokio cooperation tests. Required artifact matrix: unloaded and 1000-connection TCP/UDP throughput, 1/100/1000 RSS/VmSize/p99/idle CPU/close storm, plus Android captured-UID policy, lifecycle, logging-off and resource regression checks.
- Saturation correction: the AsyncWrite fast path uses one exact nonblocking acquisition for the requested chunk. Only after exhaustion does it enter the fair semaphore queue for one byte, then merge immediately available permits before returning a partial write. This avoids a 32 KiB FIFO head waiter and limits an abandoned write future to one retained byte until the stream is polled again, shut down, closed, or dropped. Focused tests cover partial-capacity progress and cancellation/reset reclamation.
- `.160` preflight result (2026-07-18): the complete working snapshot passed the mandatory busy check, `--locked` no-run build for the exact EasyTier, policy and vendored netstack library test binaries, and every configured focused test. This includes both new saturation tests, all dynamic queue budget/half-close/bounded-channel tests, output-capacity fairness, output-close termination, Tokio cooperation, UoT burst, mesh-only routing, DNS/FakeDNS configuration and lifecycle tests. The exact netstack test binary also passed its complete serial suite (17/17). Only pre-existing policy unused/dead-code warnings were emitted.
- Candidate dispatch manifest: source base is `8201a4a8270a173949e8fa0cf994ac7328aa46b2`; included functions are `BudgetedQueue`, per-Stack split `StreamBudgets`, bounded connection-creation channels, demand receive/send staging, exact budget reclamation and fair/cancellable AsyncWrite admission. `Cargo.lock` adds only the already-locked workspace `bytes` crate to `netstack-smoltcp`; smoltcp windows remain `0x3fff * 20`; Android selects a 32 MiB total Stream budget and other targets 64 MiB; no generated file or workflow pin changes. One automatic Linux/Android profiling workflow set is required after commit, followed by exact-SHA artifact verification and the full same-window matrix above. During workflow wait, prepare clean namespace/Android persistence fixtures without changing the in-flight snapshot.

## 2026-07-18 - Rejected 64 KiB Stream-limit artifact and replacement

- Rejected exact snapshot: `0c8894e280528f382d6689131990fef9e22466e6`; Linux run `29623951590` and Android run `29623951560` succeeded and exact artifact intake verified both runs, SHA-256 manifests, commit/target metadata, symbols/build ID, Android packages and HEV pin. The failure is runtime performance, not build provenance.
- Same-window Linux A/B: fairness-only `8201a4a8` unloaded DIRECT receive was `846.2 / 878.0 / 868.0 Mbit/s` (median `868.0`, zero retransmits). The 64 KiB dynamic-queue snapshot was `509.7 / 518.3 / 543.4 Mbit/s` (median `518.3`, zero retransmits), a `40.3%` regression. It fails the predeclared 95% gate and cannot be accepted.
- Memory evidence still validates demand allocation: at 1000 idle connections, fairness-only Leaf was `1,310,816 KiB VmSize / 32,300 KiB RSS`; the rejected snapshot was `673,940 KiB / 26,080 KiB`, reductions of approximately `48.6% / 19.3%`. Both established all connections and returned to 11 Leaf FDs after close. Baseline 1000-idle p99 was `13.86 ms`; rejected p99 was `20.75 ms` and is not accepted as parity evidence.
- Diagnosis and replacement boundary: preserving only the smoltcp `0x3fff * 20` windows is insufficient; the established Stream staging capacity is also throughput-sensitive in this pipeline. The replacement keeps demand allocation, 32 KiB chunks, direction-reserved global hard budgets, bounded channels and all lifecycle semantics, but restores each Stream direction's maximum to its former `0x3fff * 20` capacity. Empty connections still allocate no Stream payload, and aggregate active payload remains capped at 64 MiB desktop / 32 MiB Android.
- History discipline: commit `e26c9815` is the literal revert of rejected `0c8894e2`. The corrected implementation will be a separate commit on top and must repeat the complete `.160` gate and one automatic Linux/Android artifact set. Acceptance requires unloaded median at least 95% of `868.0 Mbit/s`, 1000-idle plus TCP/UDP parity, unchanged close/FD behavior, and retained VmSize/RSS improvement.
- Replacement `.160` gate: the complete corrected snapshot passed the mandatory busy check, `--locked` no-run build, every configured EasyTier/policy/netstack focused test, and the exact netstack binary's complete serial suite (17/17). The capacity assertion now locks both smoltcp sockets and both demand queues to `0x3fff * 20`, while the empty-queue test proves no payload allocation occurs before use. Lockfile, target cfg, generated files and workflow pins remain unchanged from the rejected dynamic implementation.
