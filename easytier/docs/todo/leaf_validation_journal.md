...
## 2026-07-16 - Android explicit mesh SOCKS KCP compatibility black hole

- Real user configuration used Android `10.44.0.88`, an explicit `via: mesh` SOCKS actor at `10.44.0.8:24443`, and fallback groups whose only non-domestic member was that actor. The peer advertised EasyTier `2.6.9-7080ecea`, `kcp_input: true`, and `proxy_prepare_ack_version: 1`.
- The actor warning `proxy.identity_unpinned` was not the failure: preflight classifies it as a non-blocking warning, mesh ICMP had zero loss, TCP port 24443 accepted connections, the SOCKS5 greeting returned `05 00`, and an Android manual `curl --socks5-hostname 10.44.0.8:24443 https://www.google.com/` returned HTTP 200.
- The same request through the policy TUN connected immediately and sent a 512-byte TLS ClientHello, but received no business payload and failed after about 16 seconds. A fixed-IP HTTPS request and plain HTTP request failed identically, so DNS was not the primary black-hole cause. Android nevertheless exposed an empty VPN DNS address list; the platform DNS contract remains a separate follow-up because browser DNS was not covered by the earlier IP-only iperf matrix.
- KR-side `easytier-cli -v proxy` proved the failing actor leg was `10.44.0.88 -> 10.44.0.8:24443`, state `Connected`, transport `KCP`. GOST then opened its configured next hop over QUIC. Thus generic `kcp_input` and a successful SOCKS control handshake did not prove policy-bridge payload interoperability with this older peer, and connect-time KCP fallback could not observe the later payload black hole.
- Mihomo reference: `/Users/fanli/Documents/mihomo-rev/adapter/outbound/socks5.go::{dialSocksServer,StreamConnContext,DialContext}` dials the SOCKS server through the configured dialer and retains that transport for the business stream; it does not silently select an unrelated optimization merely because the destination can accept it. Android reference: `/Users/fanli/Documents/clashmeta-android-rev/service/src/main/java/com/github/kr328/clash/service/TunService.kt::startTun` installs a virtual DNS server in `VpnService.Builder`, while `service/clash/module/TunModule.kt::attach` gives core the TUN FD and socket-protection callback.
- EasyTier intentional difference: portless managed HEV remains a versioned EasyTier-owned endpoint and keeps the validated policy KCP path. An explicit user SOCKS port may be hosted by an older peer, so its TCP bridge and private UoT control stream now use the ordinary mesh/smoltcp data plane. This trades some optional acceleration for old-peer interoperability and preserves the existing SOCKS schema, rules, credentials, fallback behavior, and managed HEV performance path.
- Remote `.160` locked GNU debug preflight passed: one KCP endpoint-selection test, all six mesh SOCKS bridge tests, and all eight mesh UDP/UoT relay tests. Exact Android real-config and browser evidence is still required from the workflow artifact before this regression is closed.
