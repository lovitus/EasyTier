...
Q0/Q 未通过，不开始 H0。H0 未通过，不实现 H。H 初期必须显式配置，不加入默认 `transport_priority`，也不替代现有 QUIC/KCP/native。
...
- [ ] 新增 `quic-brutal://` listener/connector，复用现有 QUIC/Tunnel framing。
...
- [ ] 完成 scheme、listener/connector factory、协议发布、端口索引、IPv4/IPv6 companion、socket bind/mark、生命周期和 endpoint 清理。
...
其中 `H=accept` 要求四条国内路径均通过容量门槛、新内核真实 GSO 生效、旧内核和资源门槛通过；`H=experimental` 不得进入默认优先级。
