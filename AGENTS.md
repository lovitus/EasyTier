...
- Batch candidate development before any validation build. Finish all currently known related implementation, tests, platform `cfg`, lockfile, workflow pin, configuration examples, and required documentation evidence in one working snapshot. Do not build or push after each small fix unless that fix is required to discover the next unknown.
...
- Use remote-build and workflow wait time productively. While one immutable snapshot builds, perform independent source/reference review, prepare validation commands and failure-injection matrices, inspect configuration/documentation compatibility, and stage host/device automation. Never edit the snapshot being built, start a second build for a partial follow-up, or confuse analysis against a newer working tree with evidence for the in-flight SHA.
...
- Before treating a validated snapshot as formally releasable, verify that the `easytier` Cargo version maps to a tag that does not already exist. `release.yml` requires the input to equal `v${cargo_version}` and refuses to overwrite an existing tag. Batch any required version bump with final release documentation before rebuilding; the bumped commit becomes a new exact candidate and must receive its own required validation evidence.
