...
- Android policy traffic MUST be generated by an application UID included in the active VPN UID ranges. The candidate app excludes its own UID and ADB shell traffic bypasses the app VPN, so neither is valid evidence. Use the workflow-built `easytier-android-policy-probe-debug.apk` (`com.kkrainbow.easytier.policyprobe`): uninstall any stale probe, install the exact artifact, confirm its UID is present in `dumpsys connectivity` VPN ranges, then run bounded commands through `run-as com.kkrainbow.easytier.policyprobe ...`. The probe has no runtime components or code; uninstall it after validation. Record command, UID, target, expected rule, exit code, and observed Leaf decision/result.
