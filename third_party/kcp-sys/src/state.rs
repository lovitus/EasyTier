...
impl KcpConnectionFSM {
    ...
    pub fn close<P: PacketHeaderFlagManipulator>(
        ...
        match self {
            KcpConnectionFSM::Established => *self = KcpConnectionFSM::LocalClosed,
            KcpConnectionFSM::PeerClosed => *self = KcpConnectionFSM::LastAck,
            KcpConnectionFSM::LastAck => {}
            _ => *self = KcpConnectionFSM::Closed,
        }
    ...
    fn do_handle_packet<P: PacketHeaderFlagManipulator>(
        ...
        match self {
            ...
            KcpConnectionFSM::LastAck => {
                if packet.has_rst() || (packet.has_fin() && packet.has_ack()) {
                    *self = KcpConnectionFSM::Closed;
                    Ok(())
                } else if packet.has_fin() {
                    p.set_fin(true);
