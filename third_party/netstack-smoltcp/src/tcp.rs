...
// Preserve the established per-flow burst capacity. Unlike the smoltcp socket
// buffers, these queues allocate payload only on demand and remain globally
// bounded, so idle connections do not reserve either capacity.
const STREAM_SEND_QUEUE_LIMIT: usize = DEFAULT_TCP_SEND_BUFFER_SIZE as usize;
const STREAM_RECV_QUEUE_LIMIT: usize = DEFAULT_TCP_RECV_BUFFER_SIZE as usize;
        ...
        while let Some(frame) = tcp_rx.recv().await {
            ...
            if packet.syn() && !packet.ack() {
                ...
                let control = Arc::new(SpinMutex::new(TcpSocketControl {
                    send_buffer: BudgetedQueue::new(STREAM_SEND_QUEUE_LIMIT),
                    send_waker: None,
                    recv_buffer: BudgetedQueue::new(STREAM_RECV_QUEUE_LIMIT),
                    recv_waker: None,
