...
fn recv(io: SockRef<'_>, bufs: &mut [IoSliceMut<'_>], meta: &mut [RecvMeta]) -> io::Result<usize> {
    ...
    let n = loop {
        // recvmsg updates the header's input/output lengths and flags. Reset all of them before
        // every attempt so a discarded truncated datagram cannot leave MSG_TRUNC set while a
        // later EAGAIN is misclassified as another truncated packet.
