...
#[test]
#[cfg(target_os = "macos")]
fn truncated_datagram_does_not_spin_after_eagain() {
    use std::{
        io, thread,
    ...
    send.send_to(b"oversized", recv_addr).unwrap();
    let deadline = Instant::now() + Duration::from_secs(1);
    let mut peek = [0u8; 1];
    loop {
        match recv.peek_from(&mut peek) {
            Ok(_) => break,
            Err(error) if error.kind() == io::ErrorKind::WouldBlock => {
                assert!(
                    Instant::now() < deadline,
                    "truncated datagram was not queued"
                );
                thread::yield_now();
            }
            Err(error) => panic!("failed to observe queued datagram: {error}"),
        }
    }

    // On the regression path, stale MSG_TRUNC hides EAGAIN and spins until another packet arrives.
    // A delayed rescue packet makes that failure deterministic without hanging the test process.
    let rescue = send.try_clone().unwrap();
    let rescue_thread = thread::spawn(move || {
        thread::sleep(Duration::from_millis(200));
        rescue.send_to(b"x", recv_addr).unwrap();
    });
    ...
    let result = recv_state.recv(
        ...
        slice::from_mut(&mut metadata),
    );
    rescue_thread.join().unwrap();
