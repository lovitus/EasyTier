...
usage() {
  cat <<'EOF'
Usage: leaf-packet-batch-validation.sh OPTIONS
...
Required:
  ...
  --mode MODE              legacy, batch, or fallback.
  --run N                  Run number in 1..99.

Optional:
  --fallback-worker FILE   Required for fallback mode.
...
while (( $# > 0 )); do
  case "$1" in
    ...
    --fallback-worker) fallback_worker=${2:-}; shift 2 ;;
...
for command_name in ip iperf3 setsid timeout awk find readlink basename date getconf python; do
...
if [[ "$mode" == fallback ]]; then
  [[ -n "$fallback_worker" ]] \
    || { echo "--fallback-worker is required for fallback mode" >&2; exit 2; }
  worker_bin="$fallback_worker"
fi
...
client_ns="etpb-${run_id}-c"
router_ns="etpb-${run_id}-r"
fixture_ns="etpb-${run_id}-f"
suffix=$(( $$ % 10000 ))
...
if [[ "$mode" == batch || "$mode" == fallback ]]; then
  core_args+=(--exp-feature leaf-packet-batch)
fi
...
while :; do
  ...
  if [[ -n "$core_pid" && -n "$worker_pid" && -n "$tun_name" ]]; then break; fi
...
if [[ "$mode" == batch ]]; then
  grep -Fq -- '--packet-batch' "$output/worker-cmdline.txt"
else
  if grep -Fq -- '--packet-batch' "$output/worker-cmdline.txt"; then
    echo "$mode final worker unexpectedly uses packet batch" >&2
    exit 1
  fi
fi
if [[ "$mode" == fallback ]]; then
  grep -Fq 'packet batch initialization failed; retrying legacy bridge' "$output/easytier-core.log"
fi
...
read_tun_total() {
  ...
  receive=$(ip netns exec "$client_ns" cat "/sys/class/net/$tun_name/statistics/rx_bytes")
  transmit=$(ip netns exec "$client_ns" cat "/sys/class/net/$tun_name/statistics/tx_bytes")
  printf '%s\n' "$((receive + transmit))"
}
